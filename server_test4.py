...
def order_move(player):
    ...
    #     # Dosya varsa, varsayılan sıra durumuna dönsün
    #     with open(file_path, "w") as file:
    #         file.write("X")
...
def play_game(client, data):
    ...
    # try:
    #     data = {"game": "start", "turn": True if client["name"] == "X" else False}
    #     json_data = json.dumps(data)
    #     client["socket"].sendall(json_data.encode("utf-8"))
    ...
    #         if data[0][0] == "move" and order_move(client["name"]):
    #             row, col = map(int, data[0][1].split())

    #             make_move(board, row, col, client["name"])
    #             send_move(client["name"], data[1][1])
    #             print_board()
    ...
    #     client["socket"].close()
    #     print(f"Bağlantı Kesildi {client['name'], client['address']}\n{client['socket']}")
    #     close_client(client["socket"])
    #     print(clients)
...
def handle_commands():
    while True:
        ...
        #     # 'move x y' formatında bir komutla hareket yapabilirsiniz
        #     _, x, y = command.split()
        #     x = int(x)
        #     y = int(y)
        #     # İlk müsait oyuncuya hareketi göndermek için send_move işlevini çağırın
        #     player = clients[0]["name"] if order_move(clients[0]["name"]) else clients[1]["name"]
        #     send_move(player, [x, y])
